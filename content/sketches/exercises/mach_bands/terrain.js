function Voronoi(){this.vertices=null;this.edges=null;this.cells=null;this.toRecycle=null;this.beachsectionJunkyard=[];this.circleEventJunkyard=[];
  ...
  function getDistances(vertices, p) {
    var distances = []
    vertices.forEach(v => {
      distances.push({distance: Math.pow(v.x-p.x, 2) + Math.pow(v.y-p.y, 2), vertex:v});
    });
    return distances
  }
  
  function nearest(vertices, p) {
    var distances = getDistances(vertices, p);
    distances = distances.sort((a, b) => {return a.distance - b.distance});
    return distances[0];
  }
  
  function farest(vertices, p) {
    var distances = getDistances(vertices, p);
    distances = distances.sort((a, b) => {return b.distance - a.distance});
    return distances[0];
  }
  
  function largestDistance(d) {
    var largest = 0;
    d.cells.forEach((cell) => {
      var vertices = [];
      cell.halfedges.forEach(e => {
        vertices.push(e.getEndpoint());
      });
      var farestP = farest(vertices, cell.site);
      largest = Math.max(largest, farestP.distance);
    });
    return largest;
  }
  ...
  function setup() {
    ...
    scl.input(genTerrain);
  
    speed = createSlider(1, 10, 1, 1);
    speed.position(10, 40);
    speed.style('width', '80px');
  
    strk = createCheckbox('stroke', true);
    strk.position(10, 70);
    strk.style('color', '#1EBCC5');
  
    voronoiDiagram = createCheckbox('voronoi diagram', false);
    voronoiDiagram.position(10, 100);
    voronoiDiagram.style('color', '#1EBCC5');
  
    perlin = createCheckbox('perlin', true);
    perlin.position(10, 130);
    perlin.style('color', '#1EBCC5');
  ...
  function draw() {
    ...
    if (voronoiDiagram.checked()) {
      background(255);
      diagram.cells.forEach((cell) => {
        stroke("black");
        strokeWeight(SW);
        point(cell.site.x, cell.site.y, 0);
        cell.halfedges.forEach(e => {
            beginShape(LINES);
            vertex(e.getStartpoint().x, e.getStartpoint().y, 0);
            vertex(e.getEndpoint().x, e.getEndpoint().y, 0);
            endShape();
        });
      });
    } else {
